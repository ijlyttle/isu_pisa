...
One of the factors in the PISA 2012 dataset is the number of televisions in the student's household. In the dataset, possible values were: "zero", "one", "two", or "three or more". Because there were so (relatively) few observations of "zero", for this invesitgation, the "zero" and "one" categories were combined to form a "one or less" category.

A linear regression as made for the students of each country, using the math score as the dependent variable and the number of televisions as the independent variable (as an ordered factor). Using R's `lm()` function, the direction and significance of the correlation was determined for each country. Significance is indicated by the p-value for a given regression being less than the popular 0.05.
...
```{r place_map_countries, echo=FALSE}
tags$div(
  style = "height: 400px",
  tags$p(
    tags$img(
...
The PISA dataset contains hundreds of factors; we wanted a way to make our data-exploration more efficient.

Moreover, for much of the time when the authors made their initial investigations, we (they?) were remote from each other: four team-members, in three cities across, two hemispheres. Thanks to a ten-hour dime difference, we would meet, virtually, only occasionally, and we had to make the most of our limited time.

To make our exploration easier, we developed a "Shiny" app, so that we could show each other our ideas interactively. 

"Let's look at *this* factor, combined with *that* factor!" 

"Let's look at that other factor, instead."

Shiny is a open-source tool, developed by RStudio, that allows an investigator to create a web-based appication powered by R. 

We then decided to look at only household factors. Inner factor, using boxplots. Outer factor, using violin plots. Added correlation by country.
