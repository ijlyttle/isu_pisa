...
# 1. Teachers, computers, and libraries at school make a difference in math scores

```{r shortages, fig.width=8, fig.height=12, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r shortagesbars, fig.width=4, fig.height=4, warning=FALSE, message=FALSE, echo=FALSE}
...
# 2. The gender gap in math is not universal but the reading gap is, in favor of girls
...
# 3. Starting school at an older age generated lower average math score. 
...
studyplot <- ggplot(data = study.luke, aes(ST57Q01, value, colour=variable)) + facet_wrap(~name) + stat_smooth(se=F)
studyplot
```
