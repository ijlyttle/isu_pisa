...
*** Looks like I messed up the colors for countries - need to check this again
...
```{r genderead, fig.width=8, fig.height=12, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r gendermaps2, fig.width=12, fig.height=7, warning=FALSE, message=FALSE, echo=FALSE}
...
# 2. Teachers, computers, and libraries at school make a difference in math scores
...
# 3. Starting school at an older age generates lower average math scores. 
...
For each country we have provided loess-smoothed curves of the relationship between mathematics score and the time spent studying outside of school. The most prominent feature that we observe is the decreasing benefit of studying. For every country there is evidence of a substantial positive relationship for the students who spend less time studying. However, those students who continue to study for long hours see diminishing returns for their efforts. It is possible that there may be a selection issue occuring where more talented students spend less time studying as they complete their homework faster. However, the relationship shows those spending 
...
studyplot <- ggplot(data = student.luke, aes(ST57Q01, PV1MATH)) + facet_wrap(~name) + stat_smooth(se=F)
studyplot
```
...
qplot(x = SC22Q01, y = value, facets = ~name, data = truancy.df, geom = "line", group=name) + xlab("Reported Truancy") + ylab("Math Scores") + theme_bw()
```
...
data_small <- 
  student2012 %>%
  dplyr::select(OECD, name, OUTHOURS) %>%
...
ggplot(mapping = aes(x = OUTHOURS)) + 
  ...
  geom_line(
    mapping = aes(group = name, color = OECD), 
    stat = "density", 
    alpha = 0.3,
    data = data_small %>% filter(OECD != "Albania")
  ) +
