...
qplot(x = numposs, y = value, data = df, facets = ~OECD, geom = "line", color = variable, group = group) + ylab("test scores") + xlab("number of possessions")
...
ggplot(subset(student2012.sub, !is.na(OUTHOURS)), aes(x = OUTHOURS, fill = CNT)) + geom_density(alpha = I(0.4)) + facet_wrap(~OECD) + theme(legend.position = "none")
...
qplot(x = ST15Q01, y = value, color = variable, facets = ~OECD, data = father.df, group = group, geom = "line") + xlab("father's employment status") + ylab("test scores")
```
...
qplot(x = SC22Q01, y = value, color = variable, facets = ~OECD, data = truancy.df, group = group, geom = "line") + xlab("reported truancy") + ylab("test scores")
```


```{r gendergap, echo=FALSE, warning = FALSE}
scores = student2012 %>%
...
qplot(CNT, math_difference, data = scores.df, color = OECD, size = OECD) + coord_flip() +
  ...
  geom_hline(yintercept=0, colour="grey80") + theme_bw() + scale_size_manual(values = c(4, 2.5, 2.5)) + xlab("")
